...
public class Application extends Controller {
	...
	public static Result getTrucks() {
		...
		List<Truck> groups = MorphiaObject.datastore.createQuery(Truck.class)
				.retrievedFields(false, "reviews").limit(10).asList();
		for (Truck t : groups) {
			t.reviews = null;
			t.entries = null;
		}
	...
	public static Result getTrucksByType(String genre) {
		...
		List<Truck> truck = MorphiaObject.datastore.createQuery(Truck.class)
				.filter("genre", regexp).retrievedFields(false, "reviews")
				.asList();
		// List<Truck> truck = MorphiaObject.datastore.find(Truck.class)
		// .field("genre").equal(genre).retrievedFields(false,
		// "reviews").asList();
		for (Truck t : truck) {
			t.reviews = null;
			t.entries = null;
...
//		String output = removeReviews(json);
	...
	public static Result getTopTrucks(String rank) {
		Gson gson = new Gson();
		List<Truck> truck = MorphiaObject.datastore.find(Truck.class)
				.order("-averageStar").retrievedFields(false, "reviews")
				.limit(Integer.parseInt(rank)).asList();
		for (Truck t : truck) {
			t.reviews = null;
			t.entries = null;
		}
...
//		String output = removeReviews(json);
	...
	private static Result checkin() {
		...
		if (json == null) {
			...
			Truck truck = MorphiaObject.datastore.createQuery(Truck.class)
					.retrievedFields(true, "name").field("_id")
					.equal(new ObjectId(tid)).get();
			User.Checkin checkin = new User.Checkin(time, location, tid,
					truck.name);
			// System.out.println(checkin.time);
			// create update query and operation
			String fid = json.findPath("fid").getTextValue();
			Query<User> updateQuery = MorphiaObject.datastore
					.createQuery(User.class).field("_id").equal(fid);
			UpdateOperations<User> ops = MorphiaObject.datastore
					.createUpdateOperations(User.class)
					.add("checkins", checkin);
			// update User
	...
	private static Result getHistory(String fid) {
		Gson gson = new Gson();
		User user = MorphiaObject.datastore.createQuery(User.class)
				.field("_id").equal(fid)
				.retrievedFields(false, "pwd", "facebookUser").get();
	...
	private static Result review(String truckid) {
		...
		if (json == null) {
			...
			Truck truck = MorphiaObject.datastore.createQuery(Truck.class)
					.retrievedFields(true, "reviewCount", "averageStar")
					.field("_id").equal(tid).get();
			...
			Query<Truck> qTruck = MorphiaObject.datastore
					.createQuery(Truck.class).field("_id").equal(tid);
			UpdateOperations<Truck> opsTruck = MorphiaObject.datastore
					.createUpdateOperations(Truck.class)
					.set("averageStar", average_star).inc("reviewCount")
					.add("reviews", review);
			// update truck
			MorphiaObject.datastore.update(qTruck, opsTruck);
			// create update query and operation for user
			Query<User> qUser = MorphiaObject.datastore.createQuery(User.class)
					.field("_id").equal(fid);
			...
			if (qUser.get() != null) {
				...
				UpdateOperations<User> opsUser = MorphiaObject.datastore
						.createUpdateOperations(User.class).set("checkins",
								checkins);
				// update User
